...
本项目中极其可能出现的错误有：撇与竖撇混淆；点、提、撇与捺混淆；捺与提捺混淆。除此之外，其他外形类似的笔画也可能互相混淆。更加过分的是，同一个构件的同一笔画在本项目中可能完全不统一。因此，非常欢迎闲得蛋疼的人士帮忙校对。

## 所以这有什么卵用？

可以用于实现一个类搜狗拼音/微软拼音的字形拆解反查功能，例如 `d't'shui` → `冰`，`d'd't'zhi'd't'p'd` → `渋`。
